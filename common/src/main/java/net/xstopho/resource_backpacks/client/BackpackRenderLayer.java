...
public class BackpackRenderLayer<T extends LivingEntityRenderState, M extends EntityModel<? super T>> extends RenderLayer<T, M> implements BackpackRenderer<T> {
    ...
    @Override
    public void render(PoseStack poseStack, MultiBufferSource bufferSource, int light, T renderState, float v, float v1) {
        ItemStack backpack = ((BackpackRenderState) renderState).getBackpack();
